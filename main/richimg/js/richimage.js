    ...
    cacheImg.each((i, n)=> {
        ...
        $(n).on('load', e=> {
            ...
            if (imgArr.length >= len) {
                ...
                imgArr.forEach((img, k)=> {
                    let curParent = img.curParent;
                    curParent.append("<div class='richimg-parent'></div>");
                    let parent = $(".richimg-parent").eq(k);
                    parent.append(img.target);
                    //curImg.remove();

                    parent.width(img.width);
                    parent.height(img.height);
                    $.getJSON(img.url + "?callback=" + new Date().getTime(), (data)=> {
                        img.json = data.richImgData;
                        let tags = img.json.tags;
                        tags.forEach((tag)=> {
